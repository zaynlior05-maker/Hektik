    ...
    try:
        ...
        for cc, d in DEFAULT_LEADS.items():
            if cc not in LEADS:
                ...
                # Migrate older LEADS formats if they lack 'verticals' but have root 'carriers'
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("lc|"):
        ...
        await query.edit_message_text(
            f"{d['flag']} *{d['name']} Data Hub*\n\n"
            f"Select a dynamic data vertical:",
            reply_markup=vertical_keyboard(cc),
            parse_mode="Markdown")
        return
